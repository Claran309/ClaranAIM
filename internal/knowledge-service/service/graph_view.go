...
func edgeKey(edge GraphEdge) string {
	return normalizeRelation(edge.Relation) + ":" +
		strconv.FormatInt(edge.SourceID, 10) + ">" +
		strconv.FormatInt(edge.TargetID, 10) + ":" +
		strconv.FormatInt(edge.DocumentID, 10) + ":" +
		strings.ToLower(strings.TrimSpace(edge.Evidence))
}
