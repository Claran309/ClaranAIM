...
import (
	"ClaranAIM/internal/bot-runtime-service/handler"
...
func main() {
	logger.InitService("bot-runtime-service")

	cfg, err := config.Load("config/bot-runtime-service.yaml")
	if err != nil {
		logger.Fatal("加载配置失败", "error", err)
	}

	runtimeSvc := service.NewBotRuntimeService(service.RuntimeConfig{
	...
	runtimeHandler := handler.NewBotRuntimeServiceImpl(runtimeSvc)

	r, err := etcd.NewEtcdRegistry(cfg.Etcd.Endpoints)
	if err != nil {
		logger.Fatal("创建etcd注册中心失败", "error", err)
	}
	health.CheckEtcd(cfg.Etcd.Endpoints, "bot-runtime-service")
