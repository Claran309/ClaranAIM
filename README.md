...
## to fix list
- 知识图谱里的关系还是不够多，我之前已经说过了，我希望这个关系由llm分析文档的时候建立，并在链路中一直传递，直到到达前端然后显示这个llm分析的“关系”。并且有时候llm会因为自己的短时记忆而只分析出几种同质化的关系，需要在prompt里修正一下这个问题
- agent疑似还是无法运行skill或者运行skill时间过长（我等了十分钟），请结合自己调试并日志修改bug
- 可以加一些显示的运行指定skill的标识符，就像其他agent工具一样
- 可以加一些显示的运行指定功能/tool/mcp的标识符，就像其他agent工具一样
- ocr结果有其他字段![alt text](image.png)
- im界面的“助手”界面开启的直接跟agent对话的界面应该跳转到与agent私聊而不是直接对话或者运行