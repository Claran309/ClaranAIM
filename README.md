...
## to fix list
- 管理面板的mcp部分仍然后很多重复的mcp，这些mcp除了时间戳其他都一样，看看是不是跟这个原因有关
- Grafana要加上jaeger和ELK才对
- 私聊agent时agent仍然会回复自己的消息，请结合日志排查原因并解决该问题，并自己测试直到该问题解决为止
- agent仍然无法运行skill，我注意到当前存储的skill在"storage/agent/skills/用户标识"里，没有标准的skill文件夹和SKILL.md，问题可能与此有关，请排查问题并尝试重构该模块，并自己测试直到该问题解决为止![alt text](image-7.png)
- 知识图谱仍然只有根节点-子节点的关系，实体之间没有任何关系，请修改，在我要求加根节点前这些功能都是没问题的，并自己测试直到该问题解决为止
- 知识图谱实体间关系仍然只有包含关系，请修改，并自己测试直到该问题解决为止

- 可观测性Grafana ELK Dashboard和使用学习