...
本文档只记录当前进度、剩余缺口和后续路线。详细架构、接口和历史变更见 `README.md`、`docs/AI assistent/TechArch.md`、`docs/apiDoc.md` 和 `docs/update/`。

## 2. 当前完成度总览

- [x] IM 基础服务：用户、好友、好友分组、群组、群成员、十位用户 UID、十位群号、群号加入、消息发送、编辑、撤回、已读、本地删除、历史查询、消息搜索、文件上传、手动翻译均已有后端接口或基本闭环。
- [x] IM 高级同步：WebSocket、Outbox、消息事实表、用户消息状态、上线同步、离线索引拉取、前端消息缓存、同步游标和 ACK 兼容入口已有 MVP
- [x] Agent 基础管理：Agent 创建、配置、真实系统用户身份、权限、路由规则、计费、运行、总结、问答、洞察、回复候选、会话列表和前端管理入口已落地。
- [x] Agent-Native 事件链路：Kafka/Outbox、统一 IM 事件、订阅规则、事件调度、幂等记录、私聊默认触发、群聊 @/命令/规则触发、静默记录、审计和聊天归档任务入口已具备 MVP
- [x] Memory：memory-service、`memory_facts`、用户/群/会话/session 记忆、记忆治理接口、前端记忆管理和 Agent 运行摘要沉淀已落地。
- [x] Memory 增强：Memory-RAG、Milvus 召回、候选记忆和 conversation-intelligence-service 聊天归档 MVP 已落地；已支持 LLM 提炼器、失败重试和归档任务状态页
- [x] Settings：settings-service、LLM 预设、Prompt 模板、Skill 上传/列表/详情/编辑/删除、翻译 Prompt 和 Agent 创建时选择 LLM 预设已落地。
- [x] Settings 安全：LLM API Key 和 MCP Secret 已有应用层 AES-GCM 加密存储 MVP
...
- [x] Knowledge Graph 增强：实体 canonical key 归一化、Leiden-like 社区划分、社区摘要持久化和图谱候选审核工作台已有 MVP
- [x] Tool / Skill / MCP：runtime 已有基础工具、skill-creator、用户上传 Skill 和 MCP Gateway MVP；内置 WebSearch/Memory/RAG/KnowledgeGraph/ConversationSummary 工具已统一经 MCP 暴露，远程 HTTP/SSE MCP 可配置；Agent 文件/命令工具已增加基于 workspace root 的硬沙箱
- [x] Governance：Kafka、Transactional Outbox、DTM 基础设施、创建群 DTM Saga、限流、熔断、RPC timeout、本地 INFO/ERR 日志、Prometheus、Jaeger、固定版本 Grafana、ELK 和 admin-service 全局管理台 MVP 已落地
- [x] Frontend：聊天、好友、群、Agent、Memory、Settings、RAG、Knowledge Graph、管理台、媒体预览、Markdown 渲染、Action Card MVP 已有
...
- [x] WebSocket 在线推送、Outbox 事件、上线同步、离线索引读取和前端缓存合并已接入
- [x] 客户端本地缓存已有浏览器 localStorage 最近消息缓存 MVP
- [x] 离线推送、多端完整同步、Ack/重试和乱序补偿：已补浏览器端同步游标、pending ACK 队列和 `/message/sync/ack` 兼容入口
- [ ] 厂商/浏览器离线推送、持久化设备 ACK 和更完整乱序补偿仍未完成。
...
- [x] 未读摘要、“我错过了什么”：按当前用户未读范围即时创建并处理 conversation-intelligence job，不自动标记已读。
...
- [x] Context Builder 已能读取 Agent 可见消息窗口和基础附件引用，但 RAG/Memory/Task 的深度融合仍需增强。
...
- [x] conversation-intelligence-service：可创建/处理/重试会话归档任务，可消费消息/IM 事件推进活跃会话游标，并按时间窗口或消息数阈值自动归档。
- [x] 聊天记录长期 RAG：摘要/主题块进入 rag-service，候选记忆进入 memory-service pending 区；已支持规则提炼、可选 LLM 提炼器、失败重试队列、按消息/时间范围过滤和前端归档状态页
...
- [x] API Key / MCP Secret 应用层加密存储已落地，历史明文可兼容读取
...
- [x] 实体 canonical key 归一化、Leiden-like 社区划分、社区摘要持久化和图谱候选审核已有 MVP
...
- [x] consumer 幂等和死信已有 `pkg/eventbus` 能力，并已接入基础业务指标
- [x] Prometheus、Jaeger、Grafana、ELK 本地开发栈已有 MVP：服务 `/metrics`、OTLP trace/metrics、Filebeat/Logstash/Elasticsearch/Kibana 和 Grafana overview dashboard 已配置；Grafana 镜像已固定版本并修正 Elasticsearch datasource provisioning；
- [x] 系统管理台已增加可观测性入口，可跳转 Grafana、Jaeger、Kibana 和 Prometheus Targets。
- [x] admin-service：独立 Kitex 服务，支持用户/群/媒体/Agent/记忆候选/知识候选/MCP/公告/成本/管理审计的全局聚合视图；管理台用户页已支持管理员升降权
...
- [x] Agent 上下文侧边栏、运行状态、Skill 页面和触发规则页面已有 MVP
- [x] 管理端前端：系统管理台已覆盖总览、用户、群聊、媒体、Agent、知识审核、MCP、公告、成本和审计 MVP
- [x] 文件预览、图片查看和语音播放已有基础；
